...
import {tryGrip,releaseGrip,updateGrips} from "./physics-v0103.js";
import {nearby} from "./world-v0103.js";
...
function learnBody(o,dt){
  ...
  let r=Math.max(0,forward)*13+h*2.8+Math.max(0,dy)*3.2+(o.contacts>=2 ? 0.004 : 0);if(o.foldRisk>0.12)r-=0.01;m.dense=r;m.denseAccum+=r;
...
function updatePolicy(o,dt){
  ...
  let activity=0;
  for(let i=0;i<4;i++){const leg=o.legs[i],ph=(m.phase[i]+=dt*m.policy.freq*Math.PI*2)+m.policy.phase[i];for(const j of JOINTS){const a=(leg.angles && leg.angles[j] !== undefined ? leg.angles[j] : 0),e=m.model[i][j],f=[1,Math.sin(ph),Math.cos(ph),leg.contact?1:0,a,(leg.load||0),e.forward,e.lift];let out=policyOut(m.policy.legs[i][j],f,m.policy.gain);if(m.stage===0&&m.babble.leg===i&&m.babble.joint===j)out+=m.babble.value*0.48;out=clamp(out,-1,1);let target=a+out*(j==="knee" ? 0.18 : (j==="hip" ? 0.15 : 0.10));if(j==="knee")target=THREE.MathUtils.lerp(target,leg.contact ? 0.26 : 0.40,0.05);m.command[i][j]={target,activation:0.10+0.90*Math.abs(out)};activity+=Math.abs(out)}}
  m.activity=activity/20
}
function evaluate(o,app){
  const m=o.mind;if(app.simTime-m.trialTime<8)return;const dx=o.shell.position.x-m.trialX,dz=o.shell.position.z-m.trialZ,dist=Math.hypot(dx,dz),score=m.denseAccum*1.4+dist*(m.stage===2?1.4:0.65)+o.upright*0.4-o.foldRisk*0.8;
  if(score>m.bestScore+0.015){m.bestScore=score;m.best=clone(m.policy);m.lastImprove=app.simTime;m.fragments.unshift({score,leg:Math.floor(Math.random()*4)});m.fragments=m.fragments.slice(0,12)}else m.targetPolicy=clone(m.best);
