...
import {createPhysics,createOrbsightBody,updateBodySensors,driveBody,enforceAnatomy,syncBody,destroyBody} from "./physics-v0104.js";
import {setupWorld,addObject,resetMap,updateWorld,clearObjects} from "./world-v0104.js";
import {attachMind,updateMind,cleanupMind} from "./mind-v0104.js";

const BUILD="0.10.4-collision-freecam-remove-2026-09-18";
const $=id=>document.getElementById(id);
$("status").textContent="v0.10.4 main.js loaded — initializing…";
...
function refreshUI(){
  ...
  $("jointT").innerHTML=`Self-collision: ${o.selfCollision?"ON":"OFF"}<br>`+o.legs.map(l=>`L${l.index+1}: hip ${(l.angles && l.angles.hip !== undefined ? l.angles.hip : 0).toFixed(2)} • knee ${(l.angles && l.angles.knee !== undefined ? l.angles.knee : 0).toFixed(2)} • ankle ${(l.angles && l.angles.ankle !== undefined ? l.angles.ankle : 0).toFixed(2)}${l.grip?" • GRIP":""}`).join("<br>");
...
try{
  $("status").textContent="v0.10.4 building world…";
  reset();
  $("status").textContent="v0.10.4 modular engine running";
}catch(e){
