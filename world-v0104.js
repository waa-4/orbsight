...
export function setupWorld(app){
  ...
  const grid=new THREE.GridHelper(72,72,0x34443f,0x27342f);grid.position.y=0.006;scene.add(grid);
...
export function addObject(app,type,x,z,r=0.6,y=null){
  const {scene,P}=app;x=Math.max(-33,Math.min(33,x));z=Math.max(-33,Math.min(33,z));r=Math.max(0.18,r);
  ...
  if(type==="wall"){
    ...
    const hx=type==="bed"?Math.max(0.9,r*1.4):Math.max(0.7,r*1.65),hz=type==="bed"?Math.max(0.65,r):Math.max(0.7,r*1.65),hy=type==="bed" ? 0.12 : 0.16,py=(y !== null && y !== undefined) ? y : hy;
    ...
    const hx=Math.max(0.85,r*1.5),hz=Math.max(0.7,r*1.2),hy=0.12,py=(y !== null && y !== undefined) ? y : 0.30;
    mesh=new THREE.Mesh(new THREE.BoxGeometry(hx*2,hy*2,hz*2),mat(type));mesh.rotation.z=-0.20;
    body=new CANNON.Body({mass:0,material:P.mats.object,position:cv(x,py,z)});body.addShape(new CANNON.Box(cv(hx,hy,hz)));body.quaternion.setFromEuler(0,0,-0.20);meta={hx,hz,hy,top:py+hy+0.25};
  }else if(type==="pushblock"){
    dynamic=true;const h=Math.max(0.24,r*0.65),py=(y !== null && y !== undefined) ? y : h;
    mesh=new THREE.Mesh(new THREE.BoxGeometry(h*2,h*2,h*2),mat(type));body=new CANNON.Body({mass:0.34,material:P.mats.object,shape:new CANNON.Box(cv(h,h,h)),position:cv(x,py,z),linearDamping:0.12,angularDamping:0.16});r=h;meta={hx:h,hz:h,hy:h,top:py+h};
  }else if(type==="ball"){
    dynamic=true;const rad=Math.max(0.20,r*0.72),py=(y !== null && y !== undefined) ? y : rad;
    mesh=new THREE.Mesh(new THREE.SphereGeometry(rad,20,14),mat(type));body=new CANNON.Body({mass:0.22,material:P.mats.object,shape:new CANNON.Sphere(rad),position:cv(x,py,z),linearDamping:0.03,angularDamping:0.025});r=rad;
  }else if(type==="log"){
    dynamic=true;const rad=Math.max(0.18,r*0.45),len=Math.max(0.65,r*1.6),py=(y !== null && y !== undefined) ? y : rad;
    mesh=new THREE.Mesh(new THREE.CylinderGeometry(rad,rad,len,12),mat(type));mesh.rotation.z=Math.PI/2;
    body=new CANNON.Body({mass:0.32,material:P.mats.object,position:cv(x,py,z),linearDamping:0.06,angularDamping:0.04});
    ...
    dynamic=true;const hx=Math.max(0.75,r*1.35),hy=0.09,hz=0.30,py=(y !== null && y !== undefined) ? y : 0.22;
    mesh=new THREE.Mesh(new THREE.BoxGeometry(hx*2,hy*2,hz*2),mat(type));body=new CANNON.Body({mass:0.28,material:P.mats.object,shape:new CANNON.Box(cv(hx,hy,hz)),position:cv(x,py,z),linearDamping:0.08,angularDamping:0.08});r=hx;meta={hx,hz,hy,top:py+hy};
  }else{
...
export function updateWorld(app,dt){
  ...
  for(const it of app.objects){
    ...
    if(it.dynamic&&["ball"].includes(it.type)){it.body.force.x+=Math.sin(app.simTime*0.07)*0.002;it.body.force.z+=Math.cos(app.simTime*0.05)*0.0015}
  }
}
