...
export function driveJoint(j,target,activation,dt){
  ...
  const guard=span*0.11,hard=span*0.035;
  let guardDir=0;
  ...
  const force=j.baseForce*(0.18+0.82*activation)*(1+(1-edgeN)*2.2);
...
function hardBarrier(j,dt){
  const a=jointAngle(j),rv=relAngularSpeed(j),span=j.max-j.min,soft=span*0.11;
  let target=null;
...
export function createOrbsightBody(P,scene,id,x=0,z=0){
  ...
  defs.forEach(([side,front],i)=>{
    ...
    hipMount.angularDamping=0.16;upper.angularDamping=0.12;lower.angularDamping=0.13;ankle.angularDamping=0.16;foot.angularDamping=0.27;
