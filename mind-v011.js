...
function learnBody(o,dt){
  ...
  for(const leg of o.legs){
    const dShell=Math.hypot(leg.foot.position.x-o.shell.position.x,leg.foot.position.y-o.shell.position.y,leg.foot.position.z-o.shell.position.z);
    if(dShell<0.74 && (leg.angles?.knee??0)>0.78)curled++;
  }
  r-=curled*0.009;
  if(o.foldRisk>0.12)r-=0.012;m.dense=r;m.denseAccum+=r;
